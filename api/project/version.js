export const url = "/api/project/version" 
export const version_deploy_url = "/api/project/version" 
...
export function get(that) {
    return that.$axios.get(`${url}/${that.id}`)
}

export function lastVersionDeploy(that) {
    return that.$axios.get(`${url}/${that.id}/last_version_deploy`)
}