...
export default {
    data () {
        return {
            ...
            columns: [{
                ...
                key: 'image',
                // render: (h, params) => {
                //             return h('span', params.row.spec.template.spec.containers.map(item=>item.image).join(','))
                //         },
                align: 'center'
            },{
                title: '状态',
    ...
    methods: {
        ...
        handleSearch () {
            this.loading = true
            this.$axios.$get(`${cluster.list}/${this.cluster}/stateful?namespace=${this.namespace}&labelSelector=${this.labelSelector}`).then(resp=>{
                this.loading = false 
